package com.mole.androidcodestudy.widget;
...
public class PinchImageView extends AppCompatImageView {
    ...
    /**
     * 是否需要放大居上，类似topCrop的感觉
     */
    private boolean mFitTop = false;
    ...
    public Matrix getInnerMatrix(Matrix matrix) {
        ...
        if (isReady()) {
            ...
            float scale = (float) getWidth() / getDrawable().getIntrinsicWidth();
            boolean needTopCrop = scale > (float) getHeight() / getDrawable().getIntrinsicHeight();
            //计算fit center矩阵
            if (mFitTop && needTopCrop) {
    ...
    public void setFitTop(boolean fitTop) {
        mFitTop = fitTop;
    }
     ...
     * 能否双指缩放
     */
    private boolean canPinchScale = true;
    ...
    public boolean isCanPinchScale() {
        return canPinchScale;
    }

    public void setCanPinchScale(boolean canPinchScale) {
        this.canPinchScale = canPinchScale;
    }
    ...
    private final GestureDetector mGestureDetector = new GestureDetector(PinchImageView.this.getContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onDoubleTap(MotionEvent e) {
            ...
            if (canDoubleTap && mPinchMode == PINCH_MODE_SCROLL && !(mScaleAnimator != null && mScaleAnimator.isRunning())) {
